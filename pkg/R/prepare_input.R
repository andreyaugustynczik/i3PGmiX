...
#' \item soil_model: `0` - no soil model (default); `1` - ICBM/2N; `2` - Yasso; `3` - RothC
...
#' \item peat_io :  `0` - mineral soil; `1` - peat soil
#' \item anoxic_io : `0` - no anoxic imitation; `1` - activate anoxic limitation
#' }
#'
...
prepare_input <- function(
  ...
  if (settings$beetle_dist == 1 & !identical(c("bb_cbp","bb_cst","background_infestation"),colnames(site)[12:14])){
    site_aux <- site[,1:11]
  ...
  if (settings$beetle_dist == 1){
    ...
    n_spr <- length(s_idx)
    settings$spruce_idx <- ifelse(n_spr>0,s_idx,-999)
  ...
  set_def = list(light_model = 1, transp_model = 1, phys_model = 1, height_model = 1, correct_bias = 0,
                 ...
                 dist_start = 1, spruce_idx = -999, peat_io = 0, anoxic_io = 0)
