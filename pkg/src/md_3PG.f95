...
contains
    ...
    subroutine s_3PG_f ( siteInputs, speciesInputs, forcingInputs, managementInputs, pars_i, pars_b, &
        ...
        integer(kind=c_int), dimension(20), intent(in) :: settings    ! settings for the models
        ...
        real(kind=c_double), dimension(181,n_sp), intent(in) :: pars_i
		...
		! Factor to sacle WILT to get RESP_min at 25 deg C and optimum soil moisture (/s)
		min_factor = 1.7d0*86400.d0*30.d0 
		...
		soc_int = soil_carbon_i
		ns_int  = soil_nitrogen_i
		ps_int  = soil_phosphorus_i
		   ...
		   ! Initialize accounting variables
			...
			call	run_icbm(krmax, klmax, komax(1), Yl_C, Yr_C, O_C, Yl_N, Yr_N, O_N, Litter_C, &
                ...
                if ( nut_lim .eq. int(1)) then	
                    ...
                    call compute_fertility(Nav, Pav, Un(:), Up(:), root_dist(:), root_area(:), 0.031d0, fertility(:), n_sp)
                end if
			...
			! End Yasso soil module----------------------------------------------------------------------------
				
			! RothC soil module ------------------------------------------------------------------------------

			if ( soil_model .eq. int(3) ) then

                call run_rothC(tmp_ave(ii), tmp_year, ASW, asw_max, snow_water, lai, month, soil_class, Q10_soil, Yr_C, &
                              Yl_C, dpm_ratio,resp_soc, soc, soc_int, n_sp, biom_loss_foliage, biom_loss_root, mF, mort_stress, &
                              mort_thinn, biom_foliage, stems_n, mR, biom_root, dmC, mS, biom_stem, N_deposition(ii), cn_bio, &
                              cn_hum, cp_bio, cp_hum, k_bio, k_hum, fn_l, lit_cn, lit_cp, cn_veg, cp_veg, ns, ns_int, ps, ps_int, &
                              kaps_roth_bio, kaps_roth_hum, kaps_roth_dpm, kaps_roth_rpm, npr_soil, lch_dis, excessSW, &
                              Nav, Pav, TotalCarbo, &
                              TotalCarbo_dz_10cm, TotalCarbo_dz_35cm, TotalCarbo_dz_100cm, TotalCarbo_dz_300cm, & 
                              Total_resp, TotalNitro, TotalP, Yl_input, Yr_input, &
                              cs_dpm, cs_rpm, cp_dpm, cp_rpm, cs_bio, cs_hum, cs_dpm_dz, cs_rpm_dz, &
                              cs_bio_dz, cs_hum_dz, n_inorg_T, minl_n_tot, immob_n_tot, &
                              n_gas, n_lch, p_inorg_T, minl_p_tot, immob_p_tot, p_lch, peat_io, anoxic_io, dim_cslyr, pc, cs, &
                              dz_extra, ns_dpm_rpm, soilcarb_age_pool, resp_s, resp_frac, dcs, dzsoil, lit_frac, &
                              mix_s, mix_term, zz_l, soilage_max, compress_dr, compress_bh, soil_minvol_soilt_1pt, &
                              soil_minmass_soilt_1pt, f_org)

				
				! Get N, P availability and demand

				Un(:) = 0.d0
				Up(:) = 0.d0

                !Get nitrogen and phosphorus demand
				do i=1, n_sp
					Un(i) = biom_incr_foliage(i) * Ncf(i) / 100.d0 + biom_incr_root(i) * 0.67d0 * Ncr(i) / 100.d0 + &
							(biom_incr_stem(i) + biom_incr_root(i) * 0.33d0) * Ncs(i) / 100.d0
																																	 

					Up(i) = biom_incr_foliage(i) * Pcf(i) / 100.d0 + biom_incr_root(i) * 0.67d0 * Pcr(i) / 100.d0 + &
							(biom_incr_stem(i) + biom_incr_root(i) * 0.33d0) * Pcs(i) / 100.d0
				end do
